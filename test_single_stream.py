...
app.config['UPLOADED_FILES_DEST'] = '/home/jupyter/'
    ...
    for image in images:
        if vid is None:
            if size is None:
                ...
                print("SIZEEEEE",size)
            print("NEW!!")
            vid = VideoWriter("/home/jupyter/ok.avi", fourcc, float(fps), size, is_color)
        if size[0] != image.shape[1] and size[1] != image.shape[0]:
            image = resize(image, size)
        vid.write(image)
    vid.release()
    print("SIZEEEEE*******************",size)
    os.system("ffmpeg -i /home/jupyter/ok.avi -strict -2 /home/jupyter/ok.mp4")
    return vid
...
def addvideo():
    ...
    if request.method == 'POST' and 'media' in request.files:
        start=time.time()
        os.system('rm -rf /home/jupyter/*.mp4')
        os.system('rm -rf /home/jupyter/HammadData/Test/Test/*')
        if os.path.exists('/home/jupyter/tmp.mp4'):
            os.remove("/home/jupyter/tmp.mp4")
        if os.path.exists('/home/jupyter/ok.avi'):
            os.remove('/home/jupyter/ok.avi')
        if os.path.exists('/home/jupyter/ok.mp4'):
            os.remove('/home/jupyter/ok.mp4')
        ...
        with torch.no_grad():
            ...
            for idx,ii in enumerate(outputs_var):
                ...
                for i in current:
                    ...
                    cv2.putText(i,'Frame No. = '+str(count), (20, 40),cv2.FONT_HERSHEY_SIMPLEX,0.6,(0, 0, 255), 1)
                    ...
                    frames.append(i)
                    count+=1
                print('Predicted:',ind.tolist(),'Scores',ii[ind[0]],ii[ind[1]],ii[ind[2]],ii[ind[3]])
                print()
